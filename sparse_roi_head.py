...
class SparseRoIHead(CascadeRoIHead):
    ...
    def simple_test(self,
        ...
        if self.bbox_head[-1].loss_cls.use_sigmoid:
            ...
            cls_score = cls_score.softmax(-1)[..., :-1] #80cls original
            #cls_score = cls_score.softmax(-1) #81cls
        
        #cfg = dict(type='nms', iou_threshold=0.5)
        cfg = dict(type='soft_nms', iou_threshold=0.5)
        for img_id in range(num_imgs):
            ...
            out_bboxes, keep = batched_nms(bbox_pred_per_img, scores_per_img,
                                           labels_per_img, cfg)
            out_labels = labels_per_img[keep]
            if rescale:
                scale_factor = img_metas[img_id]['scale_factor']
                out_bboxes[:, :4] /= out_bboxes[:, :4].new_tensor(scale_factor)
            det_bboxes.append(out_bboxes)
            det_labels.append(out_labels)

...
class DilatedEncoder3X(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, block_mid_channels,
                 num_residual_blocks):
        super(DilatedEncoder3X, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.lateral_conv_2 = nn.Conv2d(
                        self.in_channels//2, self.out_channels, kernel_size=1)
        self.lateral_norm_2 = BatchNorm2d(self.out_channels)

        self.lateral_conv_3 = nn.Conv2d(
                        self.in_channels//4, self.out_channels, kernel_size=1)
        self.lateral_norm_3 = BatchNorm2d(self.out_channels)
    ...
    def init_weights(self):
        ...
        caffe2_xavier_init(self.lateral_conv_2)
        caffe2_xavier_init(self.lateral_conv_3)
        caffe2_xavier_init(self.fpn_conv)
        for m in [self.lateral_norm, self.fpn_norm, self.lateral_norm_2, self.lateral_norm_3]:
            constant_init(m, 1)
    ...
    def forward(self, feature):
        ...
        out_2 = self.lateral_norm_2(self.lateral_conv_2(feature[-2]))
        out_3 = self.lateral_norm_3(self.lateral_conv_3(feature[-3]))
        out_2 = out_2 + self.up_1(out)
        out_3 = out_3 + self.up_2(out_2)
        out_4 = self.fpn_norm(self.fpn_conv(out_3))
        return self.dilated_encoder_blocks(out_4),
